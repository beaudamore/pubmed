#!/usr/bin/env python3
"""Evaluate the multimodal MedGemma text-grounding canary after a cold reload."""
...
DEFAULT_OUTPUT = (
    PROJECT_ROOT
    / "output/recovery/pubmed_oncologist_recovery_text_canary_512_medgemma_multimodal_sft/evaluation/report.json"
)
...
def generate(model: Any, processor: Any, user: str, max_new_tokens: int) -> str:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Cold-reload and evaluate the multimodal MedGemma text canary.")
    ...
    for index, case in enumerate(cases, start=1):
        response = generate(model, processor, case["user"], args.max_new_tokens)
